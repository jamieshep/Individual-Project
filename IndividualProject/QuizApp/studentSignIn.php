...
<h2>Please Login as a Student</h2>
...
<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">        <!--form for taking email and password for sign in -->
    ...
    <span class="error">* <?php echo $emailErr;?></span>                                  <!-- echo error message if fields are invalid or empty-->
    <br><br>
...
{
    $query = "SELECT * FROM `student` WHERE email_address='$email'";            //query for checking if this email address matches with one in the database
    ...
    if($rows==1 and $row = $result->fetch_assoc()){                             //if there is an email match
        ...
        //prompt user to register if sign in is unsuccessful
        echo "<div class='form'>
